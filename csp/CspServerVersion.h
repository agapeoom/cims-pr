...
#define CSP_SERVER_VERSION "0.25"

/* 버전 정보

= 버전 0.25 ( 2020년 10월 7일 ) =
 * SIP 클라이언트의 IP/Port 가 변경된 경우, 인증 정보가 정확한지 확인한다.
 * 인증되지 않은 SIP 클라이언트로부터 INVITE 수신에 대한 401 응답 전송후, 인증 정보를 포함한 INVITE 수신시 인증 정보가
유효하면 SIP 클라이언트를 로그인 처리한다.
 ...
 * SIP INVITE 요청 메시지에 대한 응답을 수신하기 전에 통화 자료구조를 저장하지 못 하여서 통화 연결 처리가 정상적으로
되지 않는 버그를 패치함
 ...
 * CspServer 설정 파일에서 TLS 연동 설정을 추가함
 ...
 * 사용자 자료구조에 존재하지 않는 수신자 전화번호에 대한 INVITE 메시지를 수신하였을 때에 IP-PBX 의 Call Route Prefix
리스트를 검사하는 기능을 추가함.
 ...
 * 만료된 사용자 삭제 기능을 추가함.
 * SIP UserAgent 헤더에 CspServer 를 추가함.
