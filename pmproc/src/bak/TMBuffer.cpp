...
bool TJitterBuffer::init(int id,int nPTime)
{
   _nSID = id;
   _nPTime = nPTime;
   _JB_StartTimeMSec = 0;
...
bool TJitterBuffer::final() //just reset buffer and parameter, not dealloc buffer
{
   _mutex.lock();
...
bool TJitterBuffer::put(const TMBuffer & buf)
{
...
bool TJitterBuffer::get(TMBuffer& buf)
{
   PiResult_T ret;
   PICO::JBFrameDesc_T desc;
