...
public:
  PRtpTrans(const std::string & name) : PHandler(name) {
  ...
  bool proc() 
  {
    ...
    while(true) {
      ...
      struct RTPHEADER * phdr = (struct RTPHEADER *)_pkt;

      if(len<=0) break;
      //printf("# %s RX PKT len=%d, ip=%s, port=%d, pt=%d, seq=%d, ts=%u\n", 
      //          name().c_str(), len, ipRmt.c_str(), portRmt,
      //          phdr->payloadtype, ntohs(phdr->seqnumber), ntohl(phdr->timestamp));
...
public:
   ...
   bool init(int sessions, const std::string locip, unsigned int locport, const std::string rmtip, unsigned int rmtport)
   {
	  ...
	  _pses  = new PRtpTrans * [sessions];
	  std::string wname; 
	  for(int i=0; i<sessions; i++)
	  {
		 std::string hname = formatStr("PRtpSes[%03d]", i+1);
		 _pses[i] = new PRtpTrans(hname);
...
int main(int argc, char ** argv)
{
   ...
   int locport = atoi(argv[3]);
   int rmtport = atoi(argv[5]);

   PAVRtpModule  * prtp = NULL;

   prtp = new PAVRtpModule("PAVRtpSimModule");

   bool bres = prtp->init(sessions, argv[2], locport, argv[4], rmtport);
   ...
   printf("# Running Media Processing Agent(rtp session = %d)!\n", sessions);
   while(true)
   {
       msleep(5000);
   }
